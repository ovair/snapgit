...
var commands = map[string]command{
	...
	"get":         {runGet, "sg get <url> [directory]", "Clone a remote repository to your local machine.\nOptionally specify a directory name.\n\nEquivalent to: git clone <url> [directory]"},
	"status":      {runStatus, "sg status", "Show the current state of your working directory and staged changes.\n\nEquivalent to: git status"},
	"add":         {runAdd, "sg add <file...>", "Stage one or more files for the next commit. Use '.' to stage everything.\n\nEquivalent to: git add <file...>"},
	...
	"diff":        {runDiff, "sg diff [staged]", "Show changes in your working directory.\nUse 'sg diff staged' to show only staged changes.\n\nEquivalent to: git diff / git diff --cached"},
	"log":         {runLog, "sg log [n]", "Show commit history as a compact, decorated graph.\nOptionally limit to the last n commits.\n\nEquivalent to: git log --oneline --graph --decorate [-n]"},
	...
	"undo":        {runUndo, "sg undo [n]", "Undo the last n commits but keep all changes staged. Defaults to 1.\n\nEquivalent to: git reset --soft HEAD~n"},
	"stash":       {runStash, "sg stash [\"message\"]", "Temporarily shelve changes in your working directory.\nOptionally add a description for the stash.\n\nEquivalent to: git stash / git stash push -m \"message\""},
	...
	"clean":       {runClean, "sg clean [--force]", "Remove untracked files from the working directory.\nShows a preview and asks for confirmation unless --force is used.\n\nEquivalent to: git clean -fd"},
	"revert":      {runRevert, "sg revert <commit>", "Revert a specific commit by creating a new commit that undoes its changes.\n\nEquivalent to: git revert <commit>"},
	"cherry-pick": {runCherryPick, "sg cherry-pick <commit>", "Apply a commit from another branch to the current branch.\n\nEquivalent to: git cherry-pick <commit>"},
...
var commandOrder = []string{
	...
	"clean", "revert", "cherry-pick", "release", "completions",
}
...
var shortDesc = map[string]string{
	...
	"diff":        "Show changes (or staged changes)",
	...
	"undo":        "Undo last commit(s) (keep changes)",
...
func printHelp() {
	...
	for _, name := range commandOrder {
		...
		label := name
		prefix := "sg " + name
		if usage := cmd.usage; len(usage) > len(prefix)+1 {
			label = name + " " + usage[len(prefix)+1:]
		}
		desc := shortDesc[name]
		fmt.Printf("  %-20s %s\n", label, desc)
	}
